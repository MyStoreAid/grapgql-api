...
import { UserBranch as UserBranchModel, Role as RoleModel} from "@mystoreaid/prisma-models";
...
export default async function userCompanyBranches(parent: any, args: UserCompanyBranchArgs): Promise<Branch[]> {
    ...
    try{
        const role: Role = await RoleModel.findOneWhere({name: "owner"});
        existingUserBranches = await UserBranchModel.findManyWhereForeignKey({ AND: 
            [ { userId: userId }, { companyId: companyId }, {roleId: role.id} ] }, { branches: true });
    }
    catch (error: any) {
